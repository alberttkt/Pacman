...
public class AreaGraph {
     ...
     * @param start (AreaNode): the starting node (one end of the edge), not null
     * @param next (AreaNode): the next one (other end of the edge), not null
     * @return (int): the edge "distance" or the edge cost
     */
    private int distanceBetween(AreaNode start, AreaNode next) {
        for (AreaNode node : start.connectedNodes) {
            if (node.equals(next))
                return 1;
        }
    ...
    private float heuristicCostEstimate(AreaNode start, AreaNode goal) {
        return DiscreteCoordinates.distanceBetween(goal.coordinates, start.coordinates);
    }
